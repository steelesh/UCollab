    ...
    <>
      <div className="absolute inset-0 flex h-full w-full flex-col items-center overflow-y-auto py-24">
        <div className="flex w-full max-w-3xl items-center border-b p-4">
          <Image
            src={transformedUser.image ?? 'https://avatar.iran.liara.run/public'}
          ...
          <div className="ml-4">
            ...
            <p className="text-accent">{transformedUser.email}</p>
            <div className="text-accent flex items-center text-sm">
              <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 256 256" className="mr-1">
                <path
                  fill="currentColor"
              ...
              <span>Joined {transformedUser.createdDate}</span>
            </div>
            {transformedUser.profile?.gradYear && (
              <div className="text-accent flex items-center text-sm">
                <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 256 256" className="mr-1">
                  <path
                    fill="currentColor"
                ...
                <span>Graduating in {transformedUser.profile.gradYear}</span>
              </div>
            )}
          </div>
        </div>
        <div className="mt-2 w-full max-w-3xl px-4">
          ...
          <div className="mt-2">{transformedUser.profile?.bio ?? 'No bio available'}</div>
          <div className="mt-2">
            <span className="font-semibold">Skills:</span>
            <ul className="list-inside list-disc">
              {transformedUser.profile?.skills && transformedUser.profile.skills.length ? (
                transformedUser.profile.skills.map((skill, index) => <li key={index}>{skill}</li>)
              ) : (
