...
export async function getRealTimePost(postId: Post["id"], userId: User["id"]): Promise<PostDetails> {
...
export async function getUserPostRating(postId: Post["id"], userId: User["id"]): Promise<number> {
  return PostService.getUserPostRating(postId, userId);
}

export async function getTrendingPosts(userId: User["id"], page = 1, limit = 12): Promise<ExplorePageData> {
  return PostService.getTrendingPosts(userId, page, limit);
}

export async function isPostBookmarked(postId: Post["id"], userId: User["id"]): Promise<boolean> {
