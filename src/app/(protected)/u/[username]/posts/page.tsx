...
async function Page({ params, searchParams }: PageProps) {
  ...
  const { success, posts } = (await getCreatedPosts()) as unknown as {
    success: boolean;
    posts: PostDetails[];
  };

  if (!success) {
    return <div>Error loading created posts.</div>;
  }

  return (
    <Container>
      <PageBreadcrumb items={[{ label: "My Posts", isCurrent: true }]} />
      <Header>
        <H1>My Posts</H1>
      </Header>
